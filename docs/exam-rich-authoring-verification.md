...
The normal source verification passed with TypeScript, 42 Vitest tests, and 30 Laravel tests / 136 assertions. The initial production-build termination during Vite chunk rendering was resolved by replacing the all-in-one CKEditor JavaScript import with scoped CKEditor package imports. The Arabic rich-text editor was re-opened successfully in the browser after that refactor, and the final standard `pnpm build` completed successfully in 15.42 seconds.
