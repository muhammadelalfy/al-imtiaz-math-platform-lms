...
import {
  ...
  isBuilderTextEntryTarget,
  preserveBuilderSpaceKey,
} from "./ExamFormBuilder";
...
describe("visual form-builder keyboard guard", () => {
  it("identifies editable text inputs, textareas, and content-editable elements", () => {
    ...
    expect(
      isBuilderTextEntryTarget({ tagName: "BUTTON" } as unknown as EventTarget)
    ).toBe(false);
  });
  ...
  it("binds the guard at the embedded builder canvas boundary used by field-card editors", () => {
    ...
    const stopPropagation = vi.fn();
    const handler = boundary.props.onKeyDownCapture as (event: {
      key: string;
