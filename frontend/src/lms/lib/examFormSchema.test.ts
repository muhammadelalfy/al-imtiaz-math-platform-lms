...
describe("convertFormSchemaToExamQuestions", () => {
  it("maps supported visual-form fields into ordered Laravel exam questions", () => {
    const result = convertFormSchemaToExamQuestions(
      JSON.stringify({
        properties: {
          choice: {
            title: "اختر الإجابة",
  ...
  it("flattens nested sections and reports unsupported field types", () => {
      ...
      JSON.stringify({
        properties: {
          algebra: {
            title: "الجبر",
            type: "object",
            properties: { solve: { title: "حل المعادلة", type: "string" } },
          },
    ...
    expect(result.questions).toEqual([
      expect.objectContaining({
        type: "essay",
        prompt_html: expect.stringContaining("الجبر — حل المعادلة"),
      }),
    ]);
  ...
  it("returns an authoring warning for empty or invalid schema JSON", () => {
    expect(convertFormSchemaToExamQuestions("{").warnings).toEqual([
      expect.stringContaining("تعذر قراءة"),
    ]);
    expect(
      convertFormSchemaToExamQuestions(JSON.stringify({ properties: {} }))
        .warnings
    ).toEqual([expect.stringContaining("أضف حقلاً")]);
  });
});
