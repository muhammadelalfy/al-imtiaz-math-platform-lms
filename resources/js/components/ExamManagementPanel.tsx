...
import {
  ...
  hasExamDraftContent,
  readExamAuthoringDraft,
  saveExamAuthoringDraft,
  type ExamAuthoringDraft,
} from "@/lib/examDraftStore";

const ExamFormBuilder = lazy(() => import("@/components/ExamFormBuilder"));

type Props = { onRefresh: () => Promise<void> };

export function appendQuestionToExam(
  current: AuthoringQuestion[],
  question: Omit<ExamQuestion, "id">
): AuthoringQuestion[] {
  return [...current, { ...question, sort_order: current.length }];
}

export type ExamTemplatePayload = Parameters<
  typeof laravelApi.createExamTemplate
>[0];

export async function persistExamTemplate(
  editingId: number | null,
  payload: ExamTemplatePayload
) {
  return editingId
    ? laravelApi.updateExamTemplate(editingId, payload)
    : laravelApi.createExamTemplate(payload);
}

export function buildExamTemplatePayload(input: {
  editingId: number | null;
  departmentId: string;
  title: string;
  grade: string;
  ...
  return {
    ...
    questions: input.editingId
      ? input.questions
      : input.questions.map(({ id: _id, ...question }) => question),
  };
}
...
export default function ExamManagementPanel({ onRefresh }: Props) {
  ...
  const [previewTemplate, setPreviewTemplate] = useState<ExamTemplate | null>(
    null
  );
  ...
  const [recoverableDraft, setRecoverableDraft] =
    useState<ExamAuthoringDraft | null>(null);
  ...
  const restoreDraft = (draft: ExamAuthoringDraft) => {
    ...
    setPrintHeader(draft.printHeader);
    setPrintFooter(draft.printFooter);
    setQuestions(draft.questions);
    setInitialQuestions(draft.questions as ExamQuestion[]);
  ...
  const save = async (event: React.FormEvent) => {
    ...
    if (!questions.length)
      return setMessage("أضف سؤالاً واحداً على الأقل قبل الحفظ.");
    setSaving(true);
    setMessage("");
  ...
  return (
    <section className="live-page exam-management mx-auto w-full max-w-[1280px]">
      <div className="page-head theme-surface flex flex-col gap-4 rounded-[20px] border p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
        <div>
          <span className="eyebrow">إدارة الامتحانات</span>
          <h2>أنشئ امتحانك بوضوح</h2>
          <p className="muted">
            ابدأ بالبيانات الأساسية، أضف الأسئلة، ثم احفظ أو انشر من قائمة
            القوالب.
          </p>
        </div>
        <div className="exam-page-head-actions flex w-full items-center justify-between gap-3 sm:w-auto sm:justify-end">
          <span className="live-count">{templates.length} قالب</span>
          <button
            type="button"
            className="secondary-button inline-flex min-h-[42px] items-center justify-center rounded-xl px-4 font-extrabold"
      ...
      {recoverableDraft && (
        <aside className="exam-draft-recovery" role="status">
          <div>
            <b>مسودة غير محفوظة متاحة</b>
            <p>
              آخر حفظ تلقائي كان قبل{" "}
              {new Intl.DateTimeFormat("ar-EG", {
                hour: "numeric",
                minute: "2-digit",
              }).format(recoverableDraft.savedAt)}
              .
            </p>
          </div>
          <span>
      ...
      <div className="exam-management-grid mt-5 grid grid-cols-1 gap-5">
        <form
          className="card theme-surface exam-authoring-card rounded-2xl border p-4 sm:p-6"
          onSubmit={save}
        >
          <div className="card-head">
            <div>
              <span className="eyebrow">
                {editingId ? "تعديل امتحان" : "امتحان جديد"}
              </span>
              <h3>{editingId ? title : "ابدأ من العنوان"}</h3>
            </div>
            <ClipboardList size={20} />
          </div>
          <details className="exam-authoring-disclosure" open>
            <summary>معلومات الامتحان الأساسية</summary>
          ...
          <details className="exam-authoring-disclosure">
            ...
            <div className="exam-form-grid">
              ...
              <label>
                تذييل الورقة المطبوع
                <input
                  value={printFooter}
                  onChange={event => setPrintFooter(event.target.value)}
                  placeholder="مثال: مع تمنياتنا بالتوفيق"
                />
              </label>
            </div>
          </details>
          <div
            className="exam-authoring-mode-switch"
            role="tablist"
            aria-label="طريقة بناء الأسئلة"
          >
            <button
              type="button"
              role="tab"
              aria-selected={authoringMode === "composer"}
              className={authoringMode === "composer" ? "active" : ""}
              onClick={() => setAuthoringMode("composer")}
            >
              الأسئلة
            </button>
            <button
              type="button"
              role="tab"
              aria-selected={authoringMode === "form-builder"}
              className={authoringMode === "form-builder" ? "active" : ""}
              onClick={() => setAuthoringMode("form-builder")}
            >
              منشئ النموذج
            </button>
          </div>
          {authoringMode === "composer" ? (
            <ExamQuestionComposer
            ...
            <Suspense
              fallback={
                <div className="exam-form-builder-loading">
                  جارٍ تجهيز منشئ النموذج...
                </div>
              }
            >
              <ExamFormBuilder
                onImport={imported => {
                  ...
                  setAuthoringMode("composer");
                  toast(
                    `تمت إضافة ${imported.length} سؤالاً من منشئ النموذج. راجعها الآن في وضع السؤال الواحد.`
                  );
                }}
              />
            </Suspense>
          ...
          <section
            className={`exam-authoring-disclosure exam-bank-disclosure${isQuestionBankOpen ? " is-open" : ""}`}
          >
            <button
              type="button"
              className="exam-disclosure-trigger"
              aria-expanded={isQuestionBankOpen}
              aria-controls="exam-question-bank-panel"
              onClick={() => setIsQuestionBankOpen(current => !current)}
            >
              <span>إضافة سؤال من بنك الأسئلة</span>
              <span aria-hidden="true">{isQuestionBankOpen ? "−" : "+"}</span>
            </button>
            {isQuestionBankOpen && (
              <div id="exam-question-bank-panel">
                <QuestionBankPanel
                  departments={departments}
                  onSelect={question => {
                    setQuestions(current =>
                      appendQuestionToExam(current, question)
                    );
                    setIsQuestionBankOpen(false);
                  }}
                />
          ...
          <div className="exam-authoring-save-state">
            <span
              className={`exam-autosave-status exam-autosave-status--${autosaveStatus}`}
            >
              {autosaveStatus === "saving"
                ? "جارٍ الحفظ التلقائي..."
                : autosaveStatus === "saved"
                  ? "تم حفظ المسودة تلقائياً على هذا الجهاز"
        ...
        <div className="card theme-surface exam-template-list rounded-2xl border p-4 sm:p-6">
          <div className="card-head">
            <div>
              <span className="eyebrow">المكتبة</span>
              <h3>القوالب الجاهزة</h3>
              <p className="muted">
                راجع القوالب أسفل نموذج الإنشاء، ثم عدّل أو عاين أو انشر من نفس
                الصفحة.
              </p>
            </div>
            <BookOpen size={20} />
      ...
      <details className="card exam-departments-card">
        <summary>
          <span>
            <span className="eyebrow">التنظيم</span>
            <b>إدارة أقسام الامتحانات</b>
          </span>
          <Target size={19} />
        </summary>
        <form className="department-form" onSubmit={saveDepartment}>
          ...
          <input
            value={deptSlug}
            onChange={event => setDeptSlug(event.target.value)}
            placeholder="slug"
