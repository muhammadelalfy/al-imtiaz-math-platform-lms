...
function geometryFromQuestion(question?: ExamQuestion): {
  shape: GeometryDiagramSpec["shape"];
  dimensions: string;
} {
  if (
    !question ||
    ...
    return { shape: "rectangle", dimensions: "width=6\nheight=4" };
  return {
    shape: question.options.shape,
    dimensions: Object.entries(question.options.dimensions)
...
export function serializeAuthoringQuestions(questions: AuthoringQuestion[]) {
  return questions.map(({ id: _id, ...question }, index) => ({
    ...question,
    sort_order: index,
  }));
}

export function replaceAuthoringQuestion(
  questions: AuthoringQuestion[],
  index: number,
  question: AuthoringQuestion
) {
  return questions.map((item, itemIndex) =>
    itemIndex === index ? question : item
  );
}

export function removeAuthoringQuestion(
  questions: AuthoringQuestion[],
  index: number
) {
...
export function moveAuthoringQuestion(
  questions: AuthoringQuestion[],
  index: number,
  direction: -1 | 1
) {
...
function normalizeQuestion(question: ExamQuestion, index: number): Draft {
  return {
    id: question.id,
    type: question.type as AuthoringQuestionType,
...
export default function ExamQuestionComposer({
  initialQuestions = [],
  onChange,
}: Props) {
  ...
  const saveDraft = () => {
    if (!draft || !draft.prompt_html.replace(/<[^>]+>/g, "").trim())
      return setError("اكتب نص السؤال أولاً.");
    if (
      draft.type === "mcq" &&
      (!Array.isArray(draft.options) || draft.options.length < 2)
    )
      return setError("أضف خيارين على الأقل للسؤال.");
    const next =
      editingIndex === null
        ? [...questions, draft]
        : replaceAuthoringQuestion(questions, editingIndex, draft);
    commit(next);
    cancel();
  };
  const remove = (index: number) =>
    commit(removeAuthoringQuestion(questions, index));
  const move = (index: number, direction: -1 | 1) =>
    commit(moveAuthoringQuestion(questions, index, direction));
  const geometry = useMemo(
    () =>
      geometryFromQuestion(
        draft?.type === "geometry" ? (draft as ExamQuestion) : undefined
      ),
    [draft]
  );
  const math = useMemo(
    () =>
      mathFromQuestion(
        draft?.type === "math" ? (draft as ExamQuestion) : undefined
      ),
    [draft]
  );
  const geometrySpec =
    draft?.type === "geometry" && isGeometryDiagram(draft.options)
      ...
      : {
          ...
          dimensions: parseGeometryDimensions(geometry.dimensions),
        };
  ...
  return (
    <div className="exam-question-composer grid gap-3">
      <div className="exam-question-composer-head theme-surface flex flex-col gap-4 rounded-2xl border p-4 sm:flex-row sm:items-start sm:justify-between sm:p-[18px]">
        <div>
          <span className="eyebrow">بناء الأسئلة</span>
          <h3>سؤال واحد في كل مرة</h3>
          <p className="muted">
            أضف السؤال واحفظه، ثم استخدم زر + لإضافة السؤال التالي.
          </p>
        </div>
        <button
          type="button"
      ...
      {questions.length === 0 && !draft && (
        <div className="exam-question-empty theme-surface rounded-xl border border-dashed px-5 py-7 text-center">
          لم تتم إضافة أسئلة بعد. ابدأ من زر «إضافة سؤال».
        </div>
      )}
      <div className="exam-question-list">
        {questions.map((question, index) => (
          <article
            className="exam-question-summary theme-surface flex flex-col gap-3 rounded-xl border p-3 sm:flex-row sm:items-center sm:justify-between"
          ...
          >
            <div>
              ...
              <div>
                <strong>
                  {questionTypeLabels[question.type as AuthoringQuestionType]}
                </strong>
                <p dangerouslySetInnerHTML={{ __html: question.prompt_html }} />
              </div>
            </div>
        ...
        <div className="exam-question-editor card theme-surface rounded-2xl border p-4 sm:p-5">
                ...
                onChange={event =>
                  setDraft(current =>
                    current
                      ? {
                          ...current,
                          type: event.target.value as AuthoringQuestionType,
                          options:
                            event.target.value === "mcq"
                              ? ["الإجابة الأولى", "الإجابة الثانية"]
                              : event.target.value === "geometry"
                                ? {
                                    shape: "rectangle",
          ...
          <label>
            ...
            <RichEditor
              ...
              onChange={value =>
                setDraft(current =>
                  current ? { ...current, prompt_html: value } : current
                )
              }
            />
          </label>
          ...
          {draft.type === "math" && (
            <div className="math-authoring-fields">
              <label>
                الترميز الرياضي
                <textarea
                  dir="ltr"
              ...
              <p className="muted">
                استخدم محرر السؤال للنص العربي، واكتب الترميز هنا ليظهر للطالب
                كجزء من السؤال الرياضي.
              </p>
            </div>
          )}
          {draft.type === "geometry" && (
            <div className="geometry-authoring-fields">
              <div>
                        ...
                        current
                          ? {
                              ...
                              options: {
                                ...
                                dimensions: parseGeometryDimensions(
                                  geometry.dimensions
                                ),
                              },
                            }
                ...
                <GeometryDiagram spec={geometrySpec} />
              </div>
              <label>
          ...
          <div className="exam-question-editor-actions flex flex-col gap-2 sm:flex-row sm:items-center">
            <button
              type="button"
              className="primary theme-primary-action inline-flex min-h-[44px] items-center justify-center gap-2 rounded-xl px-4 font-extrabold"
              onClick={saveDraft}
            >
              <Check size={15} /> حفظ السؤال
            </button>
            <button
              type="button"
              className="text-button min-h-[40px] px-3"
              onClick={cancel}
            >
              إلغاء
            </button>
