...
export function hasExamDraftContent(
  draft: Omit<ExamAuthoringDraft, "savedAt">
): boolean {
  return Boolean(
    draft.title.trim() || draft.instructions.trim() || draft.questions.length
  );
}
...
export function readExamAuthoringDraft(): ExamAuthoringDraft | null {
  try {
    ...
    return Array.isArray(draft.questions) && typeof draft.savedAt === "number"
      ? draft
      : null;
  } catch {
    return null;
...
export function saveExamAuthoringDraft(
  draft: Omit<ExamAuthoringDraft, "savedAt">
): ExamAuthoringDraft {
