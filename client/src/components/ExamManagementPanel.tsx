...
export default function ExamManagementPanel({ onRefresh }: Props) {
  ...
  return (
    <section className="live-page exam-management">
      <div className="page-head"><div><span className="eyebrow">إدارة الامتحانات</span><h2>أنشئ امتحانك بوضوح</h2><p className="muted">ابدأ بالبيانات الأساسية، أضف الأسئلة، ثم احفظ أو انشر من قائمة القوالب.</p></div><div className="exam-page-head-actions"><span className="live-count">{templates.length} قالب</span><button type="button" className="secondary-button" onClick={resetAuthoring}>امتحان جديد</button></div></div>
      <div className="exam-management-grid">
        <form className="card exam-authoring-card" onSubmit={save}>
          ...
          <details className="exam-authoring-disclosure exam-bank-disclosure"><summary>إضافة سؤال من بنك الأسئلة</summary><QuestionBankPanel departments={departments} onSelect={question => setQuestions(current => appendQuestionToExam(current, question))} /></details>
