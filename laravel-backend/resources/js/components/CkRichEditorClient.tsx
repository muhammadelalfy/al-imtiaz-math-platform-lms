...
type EditorInstance = {
  ...
  model: {
    change: (
      callback: (writer: { createText: (text: string) => unknown }) => void
    ) => void;
...
export default function CkRichEditorClient({
  value,
  onChange,
  ...
  return (
    <div className="ck-rich-editor" dir="rtl" aria-label={ariaLabel}>
      <CKEditor
        ...
        config={editorConfig}
        data={value}
        onReady={instance => setEditor(instance as unknown as EditorInstance)}
        onChange={(_event, instance) => onChange(instance.getData())}
      />
      <div className="ck-rich-editor-extras">
        <button
          type="button"
          className="ck-equation-trigger"
          onClick={() => setEquationOpen(true)}
        >
          إدراج معادلة
        </button>
        <small>
          استخدم رابط صورة عام مباشر يدعم CORS لضمان ظهور الرسم في المعاينة وملف
          PDF.
        </small>
      </div>
      {equationOpen && (
        <div
          className="ck-equation-dialog"
          role="dialog"
          aria-modal="true"
          aria-label="إدراج معادلة رياضية"
        >
          <div className="ck-equation-dialog-card">
            <div>
              <span className="eyebrow">رياضيات متقدمة</span>
              <h4>إدراج معادلة LaTeX</h4>
              <p className="muted">
                اكتب المعادلة أو استخدم لوحة الرموز؛ ستُعرض في المعاينة وملف
                PDF.
              </p>
            </div>
            <div className="mathlive-host" ref={mathfieldHostRef} />
            <label className="ck-equation-display">
              <input
                type="checkbox"
                checked={displayMode}
                onChange={event => setDisplayMode(event.target.checked)}
              />{" "}
              عرض المعادلة في سطر مستقل
            </label>
            <div className="ck-equation-actions">
              <button
                type="button"
                className="primary"
